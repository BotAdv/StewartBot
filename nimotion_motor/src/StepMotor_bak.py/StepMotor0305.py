...
class StepMotor(object):
    def __init__(self, network, node_id, eds_file):
        ...
        """
        print(f"传入的node_id值: {node_id}, 类型: {type(node_id)}")  # 调试输出
        print(f"传入的eds_file路径: {eds_file}")  # 调试输出
...
#             motor = StepMotor(self.network, i+1, './NiMotion_STM42A_V1.07.eds')
...
if __name__ == "__main__":
    ...
    #     print(f"CAN总线连接失败: {e}")
    #     exit()  # 连接失败时直接退出，避免后续操作
        
    # if network.bus:
    #     print("CAN总线已连接")
    #     motor = StepMotor(network, 1, 'E:\\Project\\python\\Bachelor\\DegreeProject\\motor\\NiMotion_STM42A_V1.07.eds')
    #     motor.initialize()  # 确保初始化电机参数
    #     motor.position_ctrl(360.0, 0.2)
    # else:
    #     print("CAN总线未连接")
