...
class StepMotor(object):
    ...
    def add_to_network(self):
        ...
        try:
            ...
            print(f"节点 {self.node_id} 已添加到网络")
        except Exception as e:
            print(f"添加节点 {self.node_id} 到网络失败: {e}")
    ...
    def download_fixed_params(self):
        ...
        try:
            ...
            print(f"电机 {self.node_id} 固定参数设置完成")
            
        except Exception as e:
            print(f"设置电机 {self.node_id} 固定参数失败: {e}")
            # 尝试恢复操作状态
            try:
                self.node.nmt.state = 'OPERATIONAL'
            except:
    ...
    def configure_rpdo_mapping(self):
        ...
        try:
            ...
            try:
                # 检查目标位置对象是否存在
                target_pos_obj = self.node.object_dictionary[0x607A]
                print(f"电机 {self.node_id}: 找到目标位置对象 0x607A")
                
                # 添加变量到RPDO
                self.node.rpdo[1].add_variable(0x607A, 0)  # Target Position
                print(f"电机 {self.node_id}: 成功添加目标位置到RPDO1")
                
            except Exception as e:
                print(f"电机 {self.node_id}: 添加目标位置到RPDO1失败 (索引方式): {e}")
                # 尝试使用名称方式
                try:
                    self.node.rpdo[1].add_variable("Target Position")
                    print(f"电机 {self.node_id}: 成功添加目标位置到RPDO1 (名称方式)")
                except Exception as e2:
                    print(f"电机 {self.node_id}: 添加目标位置到RPDO1失败 (名称方式): {e2}")
                    # 尝试其他可能的名字
                    try:
                        self.node.rpdo[1].add_variable("Target position")
                        print(f"电机 {self.node_id}: 成功添加目标位置到RPDO1 (小写名称)")
                    except Exception as e3:
                        print(f"电机 {self.node_id}: 所有添加方式都失败: {e3}")
                        return False
            
            # 设置RPDO传输类型为同步传输
            self.node.rpdo[1].trans_type = 1  # 同步传输
            ...
            try:
                self.node.rpdo.save()
                print(f"电机 {self.node_id}: RPDO配置已保存")
            except Exception as e:
                print(f"电机 {self.node_id}: 保存RPDO配置失败: {e}")
                # 尝试直接保存到RPDO对象
                try:
                    # 设置RPDO的COB-ID
                    self.node.rpdo[1].cob_id = 0x200 + self.node_id
                    print(f"电机 {self.node_id}: 设置RPDO1 COB-ID为 0x{0x200 + self.node_id:X}")
                except Exception as e2:
                    print(f"电机 {self.node_id}: 设置COB-ID失败: {e2}")
            
            self.rpdo_configured = True
            print(f"电机 {self.node_id}: RPDO映射配置完成")
            return True
            
        except Exception as e:
            print(f"电机 {self.node_id}: 配置RPDO映射失败: {e}")
            return False
        finally:
            # 确保回到操作状态
            try:
    ...
    def print_fixed_params(self):
        ...
        try:
            ...
            print("Encoder resolution   = %d" % self.node.sdo["Position encoder resolution"]["Encoder incre21ments"].raw)
    ...
    def enable(self):
        ...
        try:
            ...
            self.running = True
            self.paused = False
            print(f"电机 {self.node_id} 已使能")
            
            # 读取初始位置
            self.cur_position = self.get_current_position()
            self.target_position = self.cur_position
            
        except Exception as e:
            print(f"使能电机 {self.node_id} 失败: {e}")
            self.enabled = False
    ...
    def disable(self):
        ...
        try:
            ...
            self.running = False
            self.paused = False
            print(f"电机 {self.node_id} 已失能")
        except Exception as e:
            print(f"失能电机 {self.node_id} 失败: {e}")
    ...
    def pause(self):
        ...
        if self.enabled and not self.paused:
            try:
                ...
                time.sleep(0.01)
                self.node.sdo["Controlword"].raw = 0x0   # 暂停
                self.paused = True
                self.running = False
                print(f"电机 {self.node_id} 已暂停")
            except Exception as e:
                print(f"暂停电机 {self.node_id} 失败: {e}")
    ...
    def resume(self):
        ...
        if self.enabled and self.paused:
            try:
                ...
                self.paused = False
                self.running = True
                print(f"电机 {self.node_id} 已恢复运行")
            except Exception as e:
                print(f"恢复电机 {self.node_id} 失败: {e}")
    ...
    def toggle_pause(self):
        ...
        """
        if not self.enabled:
            print(f"电机 {self.node_id} 未使能，无法暂停")
            return
    ...
    def start_position_ctrl(self):
        ...
        try:
            ...
            self.cur_position = self.get_current_position()
            self.target_position = self.cur_position
            print(f"电机 {self.node_id} 当前位置: {self.cur_position}")

            print(f"电机 {self.node_id} 位置控制模式已启动")
        except Exception as e:
            print(f"启动电机 {self.node_id} 位置控制模式失败: {e}")
    ...
    def send_position_order(self, target_pos):
        ...
        if not self.enabled or self.paused:
            print(f"电机 {self.node_id} 未使能或已暂停，不发送位置命令")
            return
            
        try:
            # 确保RPDO已配置
            if not self.rpdo_configured:
                print(f"电机 {self.node_id} RPDO未配置，尝试配置...")
                self.configure_rpdo_mapping()
            ...
            try:
                # 检查RPDO映射中是否有变量
                if len(self.node.rpdo[1].map) > 0:
                    # 尝试直接设置RPDO变量
                    for entry in self.node.rpdo[1].map:
                        try:
                            if entry.index == 0x607A:  # 目标位置索引
                                entry.raw = target_pos
                                success = True
                                break
                        except:
                            pass
                    ...
                    if success:
                        ...
                        print(f"电机 {self.node_id}: 通过RPDO发送位置命令 {target_pos}")
                    else:
                        # 如果没有找到映射，尝试直接添加到RPDO
                        try:
                            self.node.rpdo[1][0x607A].raw = target_pos
                            self.node.rpdo[1].transmit()
                            success = True
                            print(f"电机 {self.node_id}: 通过RPDO直接发送位置命令 {target_pos}")
                        except:
                            pass
            except Exception as e:
                print(f"电机 {self.node_id}: RPDO发送失败: {e}")
            ...
            if not success:
                try:
                    ...
                    success = True
                    print(f"电机 {self.node_id}: 通过SDO发送位置命令 {target_pos}")
                except Exception as e:
                    print(f"电机 {self.node_id}: SDO发送失败: {e}")
            
            # 方式3: 尝试使用索引方式
            if not success:
                try:
                    self.node.sdo[0x607A].raw = target_pos
                    success = True
                    print(f"电机 {self.node_id}: 通过索引发送位置命令 {target_pos}")
                except Exception as e:
                    print(f"电机 {self.node_id}: 索引发送失败: {e}")
            
            # 更新当前位置
            if success:
                self.cur_position = self.get_current_position()
            else:
                print(f"电机 {self.node_id}: 所有发送方式都失败")
                
        except Exception as e:
            print(f"电机 {self.node_id} 发送位置命令失败: {e}")
    ...
    def get_current_position(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    self.cur_position = self.node.sdo[0x6064].raw  # 位置实际值索引
                except:
                    self.cur_position = 0
...
class MotorGroup(object):
    def __init__(self):
        ...
        for i in range(CAN_NODE_CNT):
            print(f"初始化电机 {i+1} (节点ID: {i+1})")
            motor = StepMotor(network=self.network, node_id=i + 1, eds_file=CAN_EDS_FILE)
            # 立即添加到网络
            motor.add_to_network()
            self.motors.append(motor)

        # 当前控制的电机编号（默认为1）
        self.current_control_motor = 1
    ...
    def start_interactive_control(self, control_motor):
        ...
        """
        # 设置当前控制的电机编号
        self.current_control_motor = control_motor
        
        print("\n" + "="*50)
        print("交互式电机控制程序")
        print("="*50)
        print("控制命令：")
        print(f"  当前控制的电机: {self.current_control_motor}")
        print("  O 键 - 启动/停止当前电机")
        print("  P 键 - 暂停/恢复当前电机")
        print("  ↑ 键 - 增加目标位置 (+500)")
        print("  ↓ 键 - 减少目标位置 (-500)")
        print("  ← 键 - 小步减少目标位置 (-100)")
        print("  → 键 - 小步增加目标位置 (+100)")
        print("  1 键 - 设置目标位置为 -2000")
        print("  2 键 - 设置目标位置为 0")
        print("  3 键 - 设置目标位置为 2000")
        print("  T 键 - 切换控制电机编号 (循环切换)")
        print("  S 键 - 显示当前电机状态")
        print("  R 键 - 重新读取当前位置")
        print("  C 键 - 清除当前电机故障")
        print("  Q 键 - 退出程序")
        ...
        # 启动键盘监听线程（不再传入参数，使用self.current_control_motor）
        ...
        try:
            while not self.exit_flag:
                ...
                # 显示状态
                internal_index = self._validate_motor_index(self.current_control_motor)
                if internal_index is not None:
                    motor = self.motors[internal_index]
                    if motor.enabled and not motor.paused:
                        # 检查是否到达目标位置
                        if motor.is_target_reached():
                            print(f"电机{self.current_control_motor}已到达目标位置: {motor.target_position}")
            ...
            print("正在退出程序...")
            # 失能当前电机
            self.disable_motor(self.current_control_motor)
            print("程序退出")
        ...
        while not self.exit_flag:
            try:
                ...
                if keyboard.is_pressed('o'):
                    self.command_queue.put(('toggle_enable', self.current_control_motor))
                    time.sleep(0.3)
                elif keyboard.is_pressed('p'):
                    self.command_queue.put(('toggle_pause', self.current_control_motor))
                    time.sleep(0.3)
                elif keyboard.is_pressed('up'):
                    self.command_queue.put(('position', self.current_control_motor, 'increase', 500))
                    time.sleep(0.1)
                elif keyboard.is_pressed('down'):
                    self.command_queue.put(('position', self.current_control_motor, 'decrease', 500))
                    time.sleep(0.1)
                elif keyboard.is_pressed('left'):
                    self.command_queue.put(('position', self.current_control_motor, 'decrease', 100))
                    time.sleep(0.1)
                elif keyboard.is_pressed('right'):
                    self.command_queue.put(('position', self.current_control_motor, 'increase', 100))
                    time.sleep(0.1)
                elif keyboard.is_pressed('1'):
                    self.command_queue.put(('position', self.current_control_motor, 'set', -2000))
                    time.sleep(0.3)
                elif keyboard.is_pressed('2'):
                    self.command_queue.put(('position', self.current_control_motor, 'set', 0))
                    time.sleep(0.3)
                elif keyboard.is_pressed('3'):
                    self.command_queue.put(('position', self.current_control_motor, 'set', 2000))
                    time.sleep(0.3)
                elif keyboard.is_pressed('s'):
                    self.command_queue.put(('status', self.current_control_motor))
                    time.sleep(0.3)
                elif keyboard.is_pressed('r'):
                    self.command_queue.put(('read_position', self.current_control_motor))
                    time.sleep(0.3)
                elif keyboard.is_pressed('c'):  # 清除故障命令
                    self.command_queue.put(('clear_fault', self.current_control_motor))
                    time.sleep(0.3)
                elif keyboard.is_pressed('t'):  # 切换控制电机编号
                    self.command_queue.put(('change_motor',))
                    time.sleep(0.3)
    ...
    def _process_command(self, command):
        ...
        elif cmd_type == 'change_motor':
            # 切换控制电机编号
            old_motor = self.current_control_motor
            self.current_control_motor += 1
            if self.current_control_motor > len(self.motors):
                self.current_control_motor = 1  # 循环回到第一个电机
            
            print(f"\n[命令] 切换控制电机: {old_motor} -> {self.current_control_motor}")
            print(f"现在控制电机 {self.current_control_motor}")
            
            # 显示新电机的状态
            self.get_motor_status(self.current_control_motor)
...
if __name__ == "__main__":
    ...
    # 检查所有电机状态
    print("检查电机状态...")
    motors.get_motor_status(None)
    
    # 设置所有电机的固定参数
    print("正在设置所有电机参数...")
    motors.set_motor_fixed_params(None)
    
    # 启动所有电机的位置控制模式
    print("正在启动所有电机的位置控制模式...")
    motors.start_position_ctrl(None)
    
    # 等待电机准备就绪
    time.sleep(1)
    
    # 启动交互式控制（默认控制电机1）
    motors.start_interactive_control(control_motor=init_num)