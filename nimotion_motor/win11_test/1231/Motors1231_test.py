...
class StepMotor(object):
    ...
    def download_fixed_params(self):
        ...
        # self.node.sdo["Stepper drive parameters"]["RunCurrentVal"].raw = 500            # 运行电流（默认值500，单位0.001A）
    ...
    def print_fixed_params(self):
        ...
        # print("RunCurrentVal      = %d" % self.node.sdo["Stepper drive parameters"]["RunCurrentVal"].raw)
...
if __name__ == "__main__":
    ...
    # 设置固定参数前先检查状态
    print("检查电机状态...")
    motors.get_motor_status(init_num)
    
    # 如果有错误，先清除
    motors.clear_motor_errors(init_num)
    
    # 设置固定参数（第一次运行时需要）
    print("正在设置电机参数...")
    motors.set_motor_fixed_params(init_num)
    
    # 启动位置控制模式
    print("正在启动位置控制模式...")
    motors.start_position_ctrl(init_num)
