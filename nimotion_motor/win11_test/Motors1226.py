...
class MotorGroup(object):
    ...
    def print_motor_params(self, motor_index):
        ...
        if internal_index is not None:
            ...
            print("Actual position = %d" % self.motors[internal_index].node.sdo["Pos Actual User Value"].raw)
            print("Target Position in motor = %d" % self.motors[internal_index].node.sdo["Target Position"].raw)
    ...
    def print_all_motors_params(self):
        ...
        for i, motor in enumerate(self.motors):
            ...
            print("Actual position = %d" % self.motors[i].node.sdo["Pos Actual User Value"].raw)
            print("Target Position in motor = %d" % self.motors[i].node.sdo["Target Position"].raw)
    ...
    def start_home_ctrl(self, motor_index=None):
        ...
        if motor_index is None:
            ...
            internal_index = self._validate_motor_index(motor_index)
            if internal_index is not None:
                self.motors[internal_index].start_home_ctrl()
    ...
    def homing(self, motor_index=None):
        ...
        if motor_index is None:
            ...
            internal_index = self._validate_motor_index(motor_index)
            if internal_index is not None:
                return self.motors[internal_index].homing()
            return False
    ...
    def get_motor_object(self, motor_index):
        ...
        internal_index = self._validate_motor_index(motor_index)
        if internal_index is not None:
            return self.motors[internal_index]
        return None
...
if __name__ == "__main__":
    ...
    motors.send_position_order(-100000,2)
    motors.start_position_ctrl(2)

    # 电机组参数打印
    motors.print_motor_params(1)
    motors.print_motor_params(2)
    ...
    # motor.print_fixed_params()
    # motor.start_position_ctrl()
    # motor.send_position_order(-80000)
    # print("Actual position = %d" % motor.node.sdo["Pos Actual User Value"].raw)