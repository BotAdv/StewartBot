...
class MotorGroup(object):
    ...
    def start_interactive_control(self, control_motor=1):
        """
        启动交互式控制
        ...
        while not self.exit_flag:
            try:
                ...
                if keyboard.is_pressed('o'):
                    ...
                    self.command_queue.put(('position', control_motor, 'decrease', 10))
                    time.sleep(0.1)
                elif keyboard.is_pressed('right'):
                    self.command_queue.put(('position', control_motor, 'increase', 10))
                    time.sleep(0.1)
    ...
    def _process_command(self, command, control_motor):
        ...
        if cmd_type == 'toggle_enable':
            ...
            if internal_index is not None:
                ...
                if action == 'increase':
                    amount = command[3]
                    new_position = motor.target_position + amount
                    print(f"\n[命令] 增加电机{motor_index}目标位置: {motor.target_position} -> {new_position}")
                    self.send_position_order(new_position, motor_index)
                    
                elif action == 'decrease':
                    amount = command[3]
                    new_position = motor.target_position - amount
                    print(f"\n[命令] 减少电机{motor_index}目标位置: {motor.target_position} -> {new_position}")
                    self.send_position_order(new_position, motor_index)
                    
                elif action == 'set':
                    position = command[3]
                    print(f"\n[命令] 设置电机{motor_index}目标位置为: {position}")
                    self.send_position_order(position, motor_index)
...
if __name__ == "__main__":
    ...
    # 启动交互式控制（默认控制电机1）
    motors.start_interactive_control(control_motor=1)